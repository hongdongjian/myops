...
export function CodexSkills() {
  ...
  const install = useMutation({
    ...
    onSuccess: () => qc.invalidateQueries({ queryKey: ['codex', 'skills'] }),
  });
        ...
        {(data?.skills ?? []).map((s) => (
          <div key={s.name} className="flex flex-wrap items-center justify-between gap-2 rounded border border-border p-2">
            ...
            <div className="flex gap-2">
              {s.installed ? (
                <Button size="sm" variant="outline" onClick={() => uninstall.mutate(s.name)} disabled={!!s.pending}>卸载</Button>
              ) : (
                <Button size="sm" onClick={() => install.mutate(s.name)} disabled={!!s.pending}>安装</Button>
              )}
            </div>
          </div>
        ))}
