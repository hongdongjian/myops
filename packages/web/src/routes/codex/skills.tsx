  ...
  return (
    <div className="grid gap-4">
      ...
      <div className="space-y-1.5">
        ...
        <Input value={form.desc} onChange={(e) => update({ desc: e.target.value })} />
      </div>
      <div className="space-y-1.5">
...
function validateForm(form: SkillForm, toast: { error: (msg: string) => void }): boolean {
...
export function CodexSkills() {
  ...
  const { data, isLoading, error } = useQuery<SkillListPayload>({
    ...
    queryFn: () => apiGet<SkillListPayload>('/api/codex/skills/list'),
    refetchInterval: 3000,
  });

  const refresh = () => {
    void qc.invalidateQueries({ queryKey: ['codex', 'skills'] });
    void qc.invalidateQueries({ queryKey: ['claude', 'skills'] });
  };

  const install = useMutation({
    mutationFn: (name: string) => apiPost('/api/codex/skills/install', { name }),
    onSuccess: refresh,
    onError: (e: Error) => toast.error(e.message),
  });

  const uninstall = useMutation({
    mutationFn: (name: string) => apiPost('/api/codex/skills/uninstall', { name }),
    onSuccess: refresh,
    onError: (e: Error) => toast.error(e.message),
  });

  const updateSingle = useMutation({
    mutationFn: (name: string) => apiPost('/api/codex/skills/update-single', { name }),
    onSuccess: () => { toast.success('Update started'); refresh(); },
    onError: (e: Error) => toast.error(e.message),
  });

  const updateAll = useMutation({
    mutationFn: () => apiPost('/api/codex/skills/update'),
    onSuccess: (res: any) => { toast.success(res?.message ?? 'Updated'); refresh(); },
    onError: (e: Error) => toast.error(e.message),
  });

  // Preset CRUD reuses Claude's shared skill-presets.yaml endpoints
  const createPreset = useMutation({
    mutationFn: (form: SkillForm) => apiPost('/api/claude/skills/preset/create', buildPayload(form)),
    onSuccess: () => {
  ...
  const updatePreset = useMutation({
    mutationFn: (form: SkillForm) => apiPost('/api/claude/skills/preset/update', buildPayload(form)),
    onSuccess: () => {
  ...
  const view = async (name: string) => {
    try {
      const res = await apiGet<SkillContent>(`/api/codex/skills/content?name=${encodeURIComponent(name)}`);
      setViewing(res);
    } catch (e) {
          ...
          {(data?.skills ?? []).map((s) => (
            <div key={s.name} className="flex flex-wrap items-center justify-between gap-2 rounded border border-border p-2">
              <div className="space-y-0.5 min-w-0">
                ...
                <div className="text-xs text-muted-foreground font-mono truncate max-w-sm">{s.repo}</div>
                {s.error ? <div className="text-xs text-destructive">{s.error}</div> : null}
              </div>
              <div className="flex gap-2 flex-shrink-0">
                {s.installed ? (
                  <>
                    <Button size="sm" variant="outline" onClick={() => updateSingle.mutate(s.name)} disabled={!!s.pending}>Update</Button>
                    <Button size="sm" variant="outline" onClick={() => uninstall.mutate(s.name)} disabled={!!s.pending}>Uninstall</Button>
                  </>
                ) : (
                  <Button size="sm" onClick={() => install.mutate(s.name)} disabled={!!s.pending}>Install</Button>
                )}
                <Button size="sm" variant="outline" onClick={() => openEdit(s)} disabled={!!s.pending}>Edit</Button>
                <Button
                  size="sm"
                  variant="outline"
                  onClick={() => {
                    if (window.confirm(`Delete skill preset "${s.name}"?`)) deletePreset.mutate(s.name);
                  }}
              ...
              <div key={name} className="flex items-center justify-between gap-2 rounded border border-border px-3 py-2">
                ...
                <div className="flex gap-2">
                  <Button
                    ...
                    variant="outline"
                    onClick={() => {
                      if (window.confirm(`Uninstall skill "${name}"?`)) uninstall.mutate(name);
                    }}
                  >
                    Uninstall
                  </Button>
                  <Button size="sm" onClick={() => { setManageForm({ ...emptyForm, name }); setManaging(true); }}>Manage</Button>
                </div>
              </div>
            ))}
          </CardContent>
      ...
      <Dialog open={adding} onOpenChange={(o) => { if (!o) { setAdding(false); setAddForm({ ...emptyForm }); } }}>
        <DialogContent className="max-w-lg">
          ...
          <DialogFooter>
            ...
            <Button onClick={() => { if (validateForm(addForm, toast)) createPreset.mutate(addForm); }} disabled={createPreset.isPending}>
      ...
      <Dialog open={!!editing} onOpenChange={(o) => { if (!o) setEditing(null); }}>
        <DialogContent className="max-w-lg">
          ...
          <DialogFooter>
            ...
            <Button onClick={() => { if (validateForm(editForm, toast)) updatePreset.mutate(editForm); }} disabled={updatePreset.isPending}>
      ...
      <Dialog open={managing} onOpenChange={(o) => { if (!o) { setManaging(false); setManageForm({ ...emptyForm }); } }}>
        <DialogContent className="max-w-lg">
          ...
          <DialogFooter>
            ...
            <Button onClick={() => { if (validateForm(manageForm, toast)) createPreset.mutate(manageForm); }} disabled={createPreset.isPending}>
              {createPreset.isPending ? 'Saving...' : 'Save'}
            </Button>
          </DialogFooter>
        </DialogContent>
