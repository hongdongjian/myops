...
export class CodexAgentsService {
  ...
  confPath(): string {
    return this.deps.paths.confPath('codex', 'AGENTS.md');
  }
  ...
  async syncStatus(): Promise<{ synced: boolean; localExists: boolean }> {
    ...
    try {
      home = await fs.readFile(this.homePath(), 'utf-8');
    } catch { /* missing */ }
    return { synced: local !== null && home !== null && local === home, localExists: local !== null };
  }
  ...
  async sync(): Promise<void> {
    ...
    try {
      ...
      throw new AppError('AGENTS_MISSING', '本地 AGENTS.md 不存在，请先保存配置', 400);
    }
